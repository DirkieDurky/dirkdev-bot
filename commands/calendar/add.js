...
export async function execute(interaction) {
    ...
    if (!isSession) {
        ...
        response = `Added session from ${formatDateTime(startDateTime)} to ${formatDateTime(endDateTime)} on ${formatDateShort(startDateTime)}`;
    } else {
        const session = parseInfo[1];
        const startDateTime = session[0];
        const endDateTime = session[1];
        await googleCalendar.createEvent(process.env.CALENDAR_ID, startDateTime, endDateTime);
        response = `Added session at ${formatDateShort(startDateTime)} from ${formatDateTime(startDateTime)} to ${formatDateTime(endDateTime)}`;
    }
